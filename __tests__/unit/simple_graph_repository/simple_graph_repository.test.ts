...
const initializeGraph = async (
  mode: 'empty' | 'vertices_only' | 'full',
): Promise<SimpleGraphRepository> => {
...
describe('Simple Graph Repository', () => {
  ...
  it('Should add edges', async () => {
    ...
    await repository.addEdge(new SimpleGraphEdge('1', '2', ['et1', 'et2'], 'E1'));
  ...
  it('Should confirm that an vertex exists', async () => {
    const repository = await initializeGraph('vertices_only');
    const exists = await repository.exists(new SimpleGraphVertex('V1', ['t1', 't2'], '1'));
  ...
  it('Should confirm that an vertex does not exists', async () => {
    const repository = await initializeGraph('vertices_only');
    const exists = await repository.exists(new SimpleGraphVertex('V100', ['t1'], '100'));
  ...
  it('Should confirm that an edge exists', async () => {
    const repository = await initializeGraph('full');
    const exists = await repository.exists(new SimpleGraphEdge('1', '3', ['et2'], 'E2'));
  ...
  it('Should confirm that an edge does not exists', async () => {
    const repository = await initializeGraph('full');
    const exists = await repository.exists(new SimpleGraphEdge('10', '1', ['et1'], 'E10'));

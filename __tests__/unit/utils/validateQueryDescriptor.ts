...
export function validateQueryDescriptor(queryDescriptor: QueryDescriptor, triples: Array<QueryTriple>): boolean {
    if (queryDescriptor.queryChain.length === triples.length) {
        for (let i = 0; i < queryDescriptor.queryChain.length; i++) {
            ...
            if (triple.leftNode.searchTerm !== resultTriple.leftNode.searchTerm ||
                !triple.leftNode.types.every((e, i) => e === resultTriple.leftNode.types[i])) {
                return false;
            }

            if (triple.relationship.direction !== resultTriple.relationship.direction ||
                ...
                !triple.relationship.types.every((e, i) => e === resultTriple.relationship.types[i])) {
                return false;
            }

            if (triple.rightNode.searchTerm !== resultTriple.rightNode.searchTerm ||
                !triple.rightNode.types.every((e, i) => e === resultTriple.rightNode.types[i])) {
                return false;
            }
        }
    } else {
